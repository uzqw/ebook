...
async function waitForServer() {
  ...
  for (let i = 0; i < 30; i++) {
    try {
      const r = await fetch(`${config.url}/api/health`)
      if (r.ok) return
    } catch (e) {
      last = e
    }
  ...
  throw new Error(`PocketBase not ready: ${last?.message || 'unknown'}`)
}
...
async function ensureSuperuser() {
  ...
  await execFileAsync(config.pbBin, [
    'superuser',
    'upsert',
...
function normalizeFields(fields) {
  return Array.isArray(fields) ? fields.map((f) => ({ ...f })) : []
}
...
async function upsertCollection(definition) {
  ...
  const found = await pb.collections.getFullList({
    filter: `name = "${definition.name}"`,
    requestKey: null,
  })
  if (!found.length) {
    console.log(`create ${definition.name}`)
  ...
  if (definition.type === 'base') {
    if (!merged.some((f) => f.name === 'created')) {
      merged.push(createdField)
      changed = true
    }
    if (!merged.some((f) => f.name === 'updated')) {
  ...
  const rulesChanged = [
    ...
    'authRule',
    'manageRule',
  ].some((k) => (definition[k] ?? null) !== (current[k] ?? null))
  if (changed || rulesChanged) {
    console.log(`update ${definition.name}`)
    return pb.collections.update(current.id, { ...current, ...definition, fields: merged })
  }
  console.log(`skip ${definition.name}`)
  return current
}
const rel = (name, collectionId, required = true) => ({
  name,
  type: 'relation',
  required,
  maxSelect: 1,
...
async function ensureCollections() {
  const users = await upsertCollection({
    ...
    fields: [text('name', true, 120)],
  })
  const books = await upsertCollection({
    name: 'books',
    type: 'base',
    ...
    fields: [
      rel('user', users.id),
      text('title', true, 240),
      text('author', false, 180),
      { name: 'description', type: 'editor', required: false, maxSize: 20000 },
      {
        name: 'file',
        type: 'file',
        required: true,
        maxSelect: 1,
        maxSize: 104857600,
        mimeTypes: ['application/pdf', 'application/epub+zip', 'application/x-mobipocket-ebook'],
      },
      num('page_count'),
      {
        name: 'parse_status',
      ...
      text('parse_error', false, 1000),
      num('current_page'),
      { name: 'toc', type: 'json', required: false, maxSize: 200000 },
      { name: 'last_read_at', type: 'date', required: false },
    ],
  })
  await upsertCollection({
    ...
    fields: [
      rel('book', books.id),
      num('page_number', true),
      { name: 'text', type: 'editor', required: false, maxSize: 200000 },
      num('width'),
      num('height'),
    ],
  })
  await upsertCollection({
    ...
    listRule: 'user = @request.auth.id',
    viewRule: 'user = @request.auth.id',
    createRule: '@request.auth.id != "" && user = @request.auth.id && book.user = @request.auth.id',
    updateRule: 'user = @request.auth.id',
    deleteRule: 'user = @request.auth.id',
    fields: [
      rel('book', books.id),
...
async function ensureDemoUser() {
  try {
    await pb.collection('users').getFirstListItem(`email = "${config.appUserEmail}"`)
    return
  } catch {}
  await pb.collection('users').create({
    name: config.appUserName,
    email: config.appUserEmail,
    password: config.appUserPassword,
    passwordConfirm: config.appUserPassword,
    verified: true,
  })
  console.log(`demo user: ${config.appUserEmail}`)
}
await waitForServer()
