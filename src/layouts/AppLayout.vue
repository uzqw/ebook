...
<template>
  <div class="shell" :class="{ 'shell--sidebar-collapsed': sidebarCollapsed }">
      ...
      <div class="sidebar-top">
        <div class="sidebar-brand flex items-start justify-between gap-3">
          <div class="sidebar-brand__copy min-w-0">
            <span
              class="block text-[10px] font-extrabold uppercase tracking-widest text-[#0f7643]/80"
              >Ebook Reader</span
            >
    ...
    <nav
      class="mobile-bottom-nav md:hidden fixed bottom-0 left-0 right-0 h-14 bg-white border-t border-emerald-500/5 flex items-center justify-around z-40 px-2 pb-safe shadow-[0_-4px_12px_rgba(0,0,0,0.03)]"
    >
      <RouterLink
        to="/books"
        class="mobile-nav-link flex flex-col items-center justify-center text-[#4a5c50]"
