    ...
    <form class="auth-card w-full max-w-md p-8" @submit.prevent="submit">
      ...
      <div class="space-y-4">
        <label class="grid gap-1.5 text-sm font-bold text-[#384c3d]"
          >昵称<Input v-model="name" autocomplete="nickname" required
        /></label>
        <label class="grid gap-1.5 text-sm font-bold text-[#384c3d]"
        ...
        <p
          v-if="error"
          role="alert"
          class="rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700"
        >
          {{ error }}
        </p>
        <Button type="submit" class="w-full" :disabled="loading"
          ><Loader2 v-if="loading" class="size-4 animate-spin" />注册并登录</Button
        >
      </div>
      <p class="mt-5 text-center text-sm text-[#384c3d]">
        已有账号？<RouterLink class="font-extrabold text-[#15803d]" to="/login"
          >返回登录</RouterLink
        >
      </p>
    </form>
